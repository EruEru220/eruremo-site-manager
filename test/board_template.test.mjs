...
test("board buttons use explicit canonical data-stamp values for selection and POST",()=>{for(const x of ['b.dataset.stamp=s','selectedStamp=b.dataset.stamp','stamp:selectedStamp','"スタンプ "+s'])assert.ok(t.includes(x),x);assert.equal(t.includes('()=>{stamp=s;'),false)});
test("board distinguishes BAD_REQUEST without changing existing 403/429 handling",()=>{for(const x of ['r.status===400&&(j.error?.code||j.error)==="BAD_REQUEST"','入力内容を確認して、もう一度お試しください','r.status===403','r.status===429'])assert.ok(t.includes(x),x)});
